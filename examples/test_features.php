...
// Select all
$all = $dm->select()->from($users)->execute();
echo "✓ SELECT * ({$all[0]['name']} is now updated)\n";

// Select specific columns
$cols = $dm->select([$users->name, $users->email])->from($users)->limit(2)->execute();
...
// eq, ne
$r = $dm->select()->from($users)->where(eq($users->name, 'Bob'))->execute();
echo "✓ eq(): Found {$r[0]['name']}\n";

$r = $dm->select()->from($users)->where(ne($users->name, 'Bob'))->execute();
echo "✓ ne(): Found " . count($r) . " users not named Bob\n";

// gt, gte, lt, lte
$r = $dm->select()->from($users)->where(gt($users->age, 28))->execute();
echo "✓ gt(): " . count($r) . " users with age > 28\n";

$r = $dm->select()->from($users)->where(gte($users->age, 28))->execute();
echo "✓ gte(): " . count($r) . " users with age >= 28\n";

$r = $dm->select()->from($users)->where(lt($users->salary, 55000))->execute();
echo "✓ lt(): " . count($r) . " users with salary < 55000\n";

$r = $dm->select()->from($users)->where(lte($users->salary, 55000))->execute();
echo "✓ lte(): " . count($r) . " users with salary <= 55000\n";

// and_, or_, not_
$r = $dm->select()->from($users)->where(
    and_(
        gte($users->age, 25),
        lte($users->age, 30)
    )
...
// like, not_like
$r = $dm->select()->from($users)->where(like($users->email, '%@test.com'))->execute();
echo "✓ like(): " . count($r) . " users with @test.com email\n";

$r = $dm->select()->from($users)->where(not_like($users->name, 'A%'))->execute();
echo "✓ not_like(): " . count($r) . " users whose name doesn't start with A\n";

// between, not_between
$r = $dm->select()->from($users)->where(between($users->salary, 50000, 60000))->execute();
echo "✓ between(): " . count($r) . " users with salary 50k-60k\n";

$r = $dm->select()->from($users)->where(not_between($users->salary, 50000, 60000))->execute();
echo "✓ not_between(): " . count($r) . " users with salary NOT 50k-60k\n";

// in_array, not_in_array
$r = $dm->select()->from($users)->where(in_array($users->name, ['Alice Updated', 'Bob', 'Charlie']))->execute();
echo "✓ in_array(): " . count($r) . " users in list\n";

$r = $dm->select()->from($users)->where(not_in_array($users->name, ['Alice Updated', 'Bob']))->execute();
echo "✓ not_in_array(): " . count($r) . " users NOT in list\n";

// is_null, is_not_null
$r = $dm->select()->from($users)->where(is_null($users->age))->execute();
echo "✓ is_null(): " . count($r) . " users with null age\n";

$r = $dm->select()->from($users)->where(is_not_null($users->age))->execute();
...
// count
$r = $dm->select([sql_count()])->from($users)->execute();
echo "✓ count(*): {$r[0]['COUNT(*)']}\n";

// count with column
$r = $dm->select([sql_count($users->age)])->from($users)->execute();
$cnt = array_values($r[0])[0];
echo "✓ count(age): {$cnt} (excludes nulls)\n";

// sum
$r = $dm->select([sql_sum($users->salary)->as('total_salary')])->from($users)->execute();
echo "✓ sum(salary): {$r[0]['total_salary']}\n";

// avg
$r = $dm->select([sql_avg($users->salary)->as('avg_salary')])->from($users)->execute();
echo "✓ avg(salary): " . round($r[0]['avg_salary'], 2) . "\n";

// min, max
$r = $dm->select([sql_min($users->age)->as('min_age'), sql_max($users->age)->as('max_age')])->from($users)->execute();
...
$r = $dm->select([$orders->product, sql_count()->as('cnt'), sql_sum($orders->amount)->as('total')])
    ->from($orders)
...
// INNER JOIN
$r = $dm->select([$users->name, $orders->product, $orders->amount])
    ->from($users)
...
// LEFT JOIN
$r = $dm->select([$users->name, sql_count($orders->id)->as('order_count')])
    ->from($users)
...
// Final count
$final = $dm->select([sql_count()->as('total')])->from($users)->execute();
