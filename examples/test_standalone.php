...
$results = $dm->select()->from($users)->order_by(desc($users->id))->execute();
...
$updated = $dm->select()->from($users)->where(eq($users->id, 1))->execute()[0];
echo "  Updated name: {$updated['name']}\n\n";

// Delete
echo "Deleting user...\n";
$dm->delete($users)->where(eq($users->id, 2))->execute();
echo "✓ User deleted\n\n";

// Final count
$count = count($dm->select()->from($users)->execute());
